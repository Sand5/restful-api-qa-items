...
The framework also supports environment variables. The mapping converts property names to uppercase and replaces dots
with underscores:
...
Ensure your JSON serialization library (e.g., Jackson) is on the classpath; otherwise Rest-Assured will fail to
serialize objects.
...
We enforce the following rules via Checkstyle:

1. **Braces Placement**
    - Opening brace `{` must be at the end of the line (`LeftCurly`).
    - Closing brace `}` must be either alone on a line or for single-line blocks (`RightCurly`).
    - Example:
      ```java
...
2. **Javadoc Requirements**
    - **Currently turned off**: Public classes, interfaces, records, and methods are not required to have Javadocs.
    - This prevents violations for missing documentation while keeping other style checks active.
...
4. **Whitespace & Formatting**
    - Enforce whitespace after keywords, around operators, and consistent empty block handling.

5. **Immutability**
    - Local variables must be declared `final` if never reassigned (`FinalLocalVariable`).

### Running Checkstyle

Checkstyle is integrated with Maven. To validate your code, run:

```bash
mvn validate
```
The build will fail if any violations are detected (excluding Javadocs).
IntelliJ with the CheckStyle-IDEA plugin highlights violations in real-time when editing files.

**Notes**

Brace formatting issues are highlighted and must be fixed manually or via IDE reformat.
Naming, whitespace, empty block, and final variable rules are actively enforced.